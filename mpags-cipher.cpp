...
int main(int argc, char* argv[])
{
  ...
  if (!InputArgs.inputFile.empty()) {

    // Open the file and check that we can read from it
    std::ifstream inputStream(InputArgs.inputFile);
    if (!inputStream.good()) {
      std::cerr << "[error] failed to create istream on file '" << InputArgs.inputFile << "'" << std::endl;
      return 1;
    }
  ...
  if ( ! InputArgs.cipher_key.empty() ) {
    ...
    for ( const auto& elem : InputArgs.cipher_key ) {
      if ( ! std::isdigit(elem) ) {
	std::cerr << "[error] cipher key must be an unsigned long integer for Caesar cipher,\n"
	          << "        the supplied key (" << InputArgs.cipher_key << ") could not be successfully converted" << std::endl;
	return 1;
      }
    }
    caesar_key = std::stoul(InputArgs.cipher_key);
  }

  // Run the Caesar cipher (using the specified key and encrypt/decrypt flag) on the input text
  std::string outputText { runCaesarCipher( inputText, caesar_key, InputArgs.encrypt ) };

  // Output the transliterated text
  if (!InputArgs.outputFile.empty()) {

    // Open the file and check that we can write to it
    std::ofstream outputStream(InputArgs.outputFile);
    if (!outputStream.good()) {
      std::cerr << "[error] failed to create ostream on file '" << InputArgs.outputFile << "'" << std::endl;
      return 1;
    }
